...
export function Eyebrow({ children }: { children: ReactNode }) {
  return (
    <p className="mb-4 font-mono text-xs uppercase tracking-[0.18em] text-teal-600">
      {children}
    </p>
  );
}
      ...
      <Container className="flex h-16 items-center justify-between gap-6">
        <a href="#top" className="flex items-center gap-2.5 text-white">
          ...
          <span className="text-[17px] font-semibold tracking-tight">
            Easy<span className="text-teal-400">CRM</span>
          </span>
        </a>
...
export function Footer() {
  return (
    <footer className="mt-auto border-t border-slate-700 bg-slate-950 py-12 text-white/55">
      <Container>
        <div className="flex flex-col gap-8 sm:flex-row sm:items-center sm:justify-between">
          ...
          <nav className="flex flex-wrap gap-x-7 gap-y-3 text-sm">
            ...
            <a href="mailto:admin@saatvikminchem.com" className="hover:text-white">
              Contact
            </a>
          </nav>
        </div>
